...
do
	...
	db2 "select * from ($sql)a" |sed -n '4p'|awk '{print $1}' >rtrninfo
	while read rtrninfo
	do
        	isSuccess=$rtrninfo
	done <rtrninfo
...
do
	#加工数据
	db2 "call p.get_rtrninfo($myDays)"
	#核对数据
	sql="select case when a.bf=b.bf and a.js=b.js then 'Y' else 'N' end from 
		(select sum(acctamt) bf,count(distinct chdrnum) js from administrator.hn_rtrninfo 
	        where trandate between decimal(current date - day(current date -1 day) days) and decimal(current date)) a,
		(select sum(acctamt) bf,count(distinct chdrnum) js from administrator.v_rtrn_day
	        where trandate between decimal(current date - day(current date -1 day) days) and decimal(current date)) b"
	db2 "select * from ($sql)a" |sed -n '4p'|awk '{print $1}' >rtrninfo
	while read rtrninfo
	do
        	isSuccess=$rtrninfo
	...
	if [ $isSuccess = "Y" ]; then
		#记录结束时间
        	...
        	#计算时间差
	        run_time=$[ end - start ]
		checkState rtrninfo 4
	else
		checkState rtrninfo 3
                getTime=$[getTime+1]
