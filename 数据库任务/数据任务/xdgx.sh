...
do
	...
	#防止"3"错误
	db2 "load from test.txt of del terminate into administrator.hn_xdgx"
	#清空表
	db2 "alter table administrator.hn_xdgx activate not logged initially with empty table"
	#加工数据
	db2 "load from c1 of cursor insert into administrator.hn_xdgx"
	#核对数据
	if [ $? == 0 ]; then 
        	isSuccess='Y'
	fi
