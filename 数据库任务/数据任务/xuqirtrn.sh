...
do
	sql="select case when count(*)>=1 then 'Y' else 'N' end
        	from info.taskmessage
                	where date(info_time)=current date -1 days and message='数据正常!' and tablename='xuqidetail'"
	db2 "select * from ($sql)a" |sed -n '4p'|awk '{print $1}' >xuqirtrn
	while read xuqirtrn
	do
        	isSuccess=$xuqirtrn
	done <xuqirtrn
...
do
	#加工数据
	db2 "call p.get_xuqirtrn($myDays)"
	#核对数据
	sql="select case when a.bf=b.bf and a.js=b.js then 'Y' else 'N' end from 
		(select sum(acctamt) bf,count(distinct chdrnum) js from administrator.hn_xuqirtrn 
	        where trandate between decimal(current date - day(current date -1 day) days) and decimal(current date)) a,
		(select sum(acctamt) bf,count(distinct chdrnum) js from (select distinct * from administrator.ipe_xuqirtrn
	...
	if [ $isSuccess = "Y" ]; then
		#记录结束时间
        	...
        	#计算时间差
	        run_time=$[ end - start ]
		checkState xuqirtrn 4
	else
		checkState xuqirtrn 3
                getTime=$[getTime+1]
