...
do
	#清空表
	db2 "alter table administrator.hn_dv_bank activate not logged initially with empty table"
	#加工数据
	db2 "load from dv_bank of del insert into administrator.hn_dv_bank"
	#判断是否成功
	if [ $? = 0 ]; then
		isSuccess="Y"
		#记录结束时间
        	...
        	#计算时间差
	        run_time=$[ end - start ]
		checkState dv_bank 4
	else
		checkState dv_bank 3
                getTime=$[getTime+1]
