...
#业绩
sql_yj="""
delete from administrator.hn_kpi_info_bnk where category='{category_zh}' and series='渠道' and kpi_name='{kpi_name}' and kpi_mon={setMon};
insert into administrator.hn_kpi_info_bnk
with mytable as(
SELECT 
...
sql_ycrl="""
delete from administrator.hn_kpi_info_bnk where category='{category_zh}' and series='{series}' and kpi_name='{kpi_name}' and kpi_mon={setMon};
insert into administrator.hn_kpi_info_bnk
with this as(
select  
...
while(dateStart<=currentDate):
	...
	if(ifIsTrue==0):
		#当前日期-指定天数
		lastMon=datetime.datetime.now()-datetime.timedelta(days=10)
...
#--------当日业绩--------------------------------------------------------------------------------
#删除当日业绩
cur.execute("delete from administrator.hn_kpi_info_bnk where length(trim(char(kpi_mon)))=8".decode('utf-8')) 
cur.commit()
...
#-----------------------------------------------------------------------------------------------------
print("加工中支业绩")
cur.execute("delete from administrator.hn_kpi_info_bnk where series='中支' and kpi_name not in('月初人力','当前人力')".decode('utf-8'))
curr_sql="""
insert into administrator.hn_kpi_info_bnk 
select 
        category,
...
print("加工县域业绩")
cur.execute("delete from administrator.hn_kpi_info_bnk where series='县域'".decode('utf-8'))
curr_sql="""
insert into administrator.hn_kpi_info_bnk 
select 
        category,
...
#--------------------------------------------------------------------------------------------------------
print("加工银保业绩")
cur.execute("delete from administrator.hn_kpi_info_bnk where category='银保'".decode('utf-8'))
curr_sql="""
insert into administrator.hn_kpi_info_bnk 
select 
        '银保',
...
"""
cur.execute(curr_sql.decode('utf-8')) 
cur.execute('update administrator.hn_kpi_info_bnk set branch=trim(branch)'.decode('utf-8')) 
